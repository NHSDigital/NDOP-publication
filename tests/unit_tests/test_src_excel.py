...
import pandas as pd
import pytest
import ndop.preprocessing.ndop_clean as clean
...
class TestNDOPCleaningFuncs:
    ...
    def nhs_numbers(self):
        return pd.DataFrame(
            {"NHS_Number": ["5555555555", "111111", "2222", "99999", None]}
        )
    ...
    def test_ndop_removing_invalid_nhs_numbers_starting_with_9(self, nhs_numbers):
        ...
        actual = clean.remove_nhs_number_starting_with_9(nhs_numbers).reset_index(
            drop=True
        )

        expected = pd.DataFrame(
            {"NHS_Number": ["5555555555", "111111", "2222", None]}
    ...
    def test_ndop_removing_invalid_nhs_numbers_defined_in_config(self, nhs_numbers):
        ...
        actual = clean.remove_nhs_number_which_are_invalid(nhs_numbers).reset_index(
            drop=True
        )

        expected = pd.DataFrame(
            {"NHS_Number": ["111111", "2222", "99999", None]}
        ).reset_index(drop=True)
    ...
    def test_empty_gender_columns_filled_with_unknown_in_none_values(
    ...
    def test_remove_deceased_patients_from_dataframe(self, date_of_death):
        ...
        actual = clean.remove_deceased_patients(
            date_of_death, datetime.date(2022, 6, 1)
        )
